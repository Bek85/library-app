...
export default class SeederService extends Service {
  ...
  *seedRandomLibraries(volume) {
    ...
    const listOfNewRandomLibraryPromises = range(counter).map(() =>
      this._createAndSaveRandomLibrary()
    );
  ...
  *seedRandomAuthorsWithBooks(volume) {
    ...
    if (!libraryRecords.length)
      libraryRecords = [yield this._createAndSaveRandomLibrary()];

    const counter = parseInt(volume, 10);
    const listOfNewRandomAuthorPromises = range(counter).map(() =>
      this._createAndSaveRandomAuthor()
    );
  ...
  *deleteAuthorsAndTheirBooks() {
    ...
    const destroyBookPromises = authorRecords.map((author) =>
      this._destroyAll(author.books)
    );
